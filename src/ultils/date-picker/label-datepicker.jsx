...
class LabelDatePicker extends Component {
    ...
    constructor(props) {
        super(props);
        let propsDate = this.props.selected, def,
        ...
        if (typeof propsDate === 'string') {
            if (DateUtilities.isCorrectDateTime(propsDate)) {
                const strDate = propsDate.substring(0, 10),
    ...
    componentDidMount() {
        ...
        this.assignRefs['inputDateTimePicker' + this.state.id].click();
    };
    ...
    componentWillReceiveProps(nextProps) {
        let propsDate = nextProps.selected, def,
            hourTime = "00", minuteTime = "00";
        if (typeof propsDate === 'string') {
            if (DateUtilities.isCorrectDateTime(propsDate)) {
                const strDate = propsDate.substring(0, 10),
    ...
    onChangeDate = (event) => {
        var self = this;
        const strDateTime = event.target.value.trim();
    ...
    onChangeHourTime = (hourTime) => {
        var self = this;
        const strDateTime = DateUtilities.toString(this.state.selected) + " " + ('0' + hourTime).slice(-2) + ":" + this.state.minuteTime;
        this.setState({
    ...
    onChangeMinuteTime = (minuteTime) => {
        var self = this;
        const strDateTime = DateUtilities.toString(this.state.selected) + " " + this.state.hourTime + ":" + ('0' + minuteTime).slice(-2)
        this.setState({
    ...
    show = (event) => {
        const elInput = this.assignRefs['inputDateTimePicker' + this.state.id],
            elCalendar = this.assignRefs['calendar_' + this.state.id],
    ...
    render() {
        ...
        return (
            <div className="ardp-date-picker label-date-picker" onMouseLeave={this.props.onMouseLeave}>
                ...
                <input type="text"
                       ref={function (el) {self.assignRefs['inputDateTimePicker' + self.state.id] = el}}
